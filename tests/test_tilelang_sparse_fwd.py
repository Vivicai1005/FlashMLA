...
def get_flop(p: TestParam) -> float:
    flop = 2 * (p.h_q * p.d_qk * p.topk + p.h_q * p.d_v * p.topk) * p.b * p.s_q
    return flop
...
def run_test(p: TestParam) -> bool:
    ...
    if isinstance(ans, (tuple, list)):
        ans_out = ans[0]
    elif isinstance(ans, torch.Tensor):
        ans_out = ans
    else:
        raise TypeError("tilelang_sparse_fwd returned unsupported output type")

    ans_out = ans_out.squeeze(0)
    torch.cuda.synchronize()

    if p.benchmark:
        flop = get_flop(p)
        prefill_ans_time = (
            triton.testing.do_bench(run_ans, warmup=10, rep=20) / 1000  # type: ignore
        )
    ...
    if p.check_correctness:
        torch.cuda.synchronize()
        ref_out = reference_torch(p, t, sm_scale)
        torch.cuda.synchronize()
        return check_is_allclose(
